...
class Mymodel(KerasPilot):
    ...
    def create_model(self):
        img_seq_shape = (self.seq_length,) + self.input_shape
        ...
        x = TD(Dropout(drop_out))(x)
        x = TD(Convolution2D(64, (5, 5), strides=(1, 1), activation='relu'))(x)
        x = TD(Dropout(drop_out))(x)
        x = TD(Convolution2D(64, (5, 5), strides=(1, 1), activation='relu'))(x)
        x = TD(Dropout(drop_out))(x)
        x = TD(Convolution2D(64, (5, 5), strides=(2, 2), activation='relu'))(x)
        x = TD(Dropout(drop_out))(x)
        x = TD(MaxPooling2D(pool_size=(2, 2)))(x)
        x = TD(Flatten(name='flattened'))(x)

        x = LSTM(100, return_sequences=True, name="LSTM_seq")(x)
        x = Dropout(.1)(x)
        x = LSTM(50, return_sequences=False, name="LSTM_fin")(x)
        x = Dropout(.1)(x)
        outputs = []
        outputs.append(Dense(1, activation='linear', name="angle")(x))
        outputs.append(Dense(1, activation='linear', name="throttle")(x))
