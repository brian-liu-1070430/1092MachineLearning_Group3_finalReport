...
import donkeycar.parts.cv as dkcv2
...
def drive(cfg, model_path=None, model_type=None):
    ...
    gray = dkcv2.ImgGreyscale()
    cut = dkcv2.Imgcut(x=0,y=40)
    car.add(gray,inputs=['cam/image_array'],outputs=['cam/image_array'],threaded=False)
    car.add(cut, inputs=['cam/image_array'], outputs=['cam/image_array'], threaded=False)
    # add controller
